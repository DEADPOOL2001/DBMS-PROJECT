...
var wait = require('../models/waiting');
var currTeacherID ;
var currStudentID;
var currCourseID;
var currTestID;
...
router.get("/ADMIN",function(req, res) {
  res.render('admin_log',{message:message});
  message="";
});
...
router.get("/admin1",function(req, res) {
  wait.find({},(err,data)=>{
    ...
    res.render('page',{teacher:data});
    waitlist=data;
    console.log(waitlist);
  });

});
router.post("/1234",function(req, res) {
  console.log(req.body);
  let ind=parseInt(req.body.number, 10);
  console.log(waitlist[ind]);
  var newPerson = new teacher({
    fName: waitlist[ind].fName,
    Lname: waitlist[ind].Lname,
    uname:"no",
    email:waitlist[ind].email,
    Address : waitlist[ind].Address,
    Password :waitlist[ind].Password,
    Country : waitlist[ind].Country,
    State : waitlist[ind].State,
    Zip_code : waitlist[ind].Zip_code
  });
  ...
  wait.deleteOne({  email: waitlist[ind].email }, function(err, result) {
  if (err) {
    res.send(err);
  }
  });
...
router.get("/announce",function(req, res) {
  console.log(course1);
  mess.find({courseID: course1._id},(err,data)=>{
    data.reverse();
    if(err)
    console.log(err);
    else
    res.render('announce',{course:course1,array:data});
  });
  console.log(messArray);
});
router.get("/announce1",function(req, res) {
  console.log(course2);
  mess.find({courseID: course2._id},(err,data)=>{
    data.reverse();
    if(err)
    console.log(err);
    else
    res.render('announce1',{course:course2,array:data});
    console.log(data);
  });
  console.log(messArray);
});
router.post("/announce",function(req, res) {
  console.log(req.body);
  var link="file:///C:/Users/hp/Downloads/2.zip";
  var hi=req.body.zip;
  console.log(link);
  var newMess= new mess({
    courseID:course1._id,
    name:course1.teachName,
    head:req.body.head,
    body :req.body.body,
    link:link
  });
  newMess.save((err, Person) => {
    if (err)
      console.log(err);
    else
      console.log('Success');
...
router.post("/announce1",function(req, res) {
  console.log(req.body);
  var newMess= new mess({
...
router.post("/teach_reg",function(req,res){
  ...
  student.findOne({ email: personInfo.email }, (err1, data1) => {
    ...
    wait.findOne({ email: personInfo.email }, (err1, data1) => {
          console.log(data1);
          flag=data1;
      });
  teacher.findOne({ email: personInfo.email }, (err, data) => {
				if (!data && !flag) {
          var newPerson = new wait({
            fName: personInfo.fname,
            Lname: personInfo.lname,
            uname:"no",
            email:personInfo.email,
